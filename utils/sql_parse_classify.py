...
def deprecated_query_has_join(sql_query: str) -> bool:
    """
...
def contain_set_operation(parsed_sql: sqlparse.sql.TokenList) -> bool:
    SET_OPERATIONS_ENUM = ['union', 'intersect', 'except']
    for token in parsed_sql.flatten():  # 使用.flatten()以简化迭代并涵盖更深层次的结构
        if token.ttype is Token.Keyword:  # 判断是否为关键字类型
            if any(op in token.value.lower() for op in SET_OPERATIONS_ENUM):  # 不区分大小写比较
                return True                                                                                                                                                      
    return False


def deprecated_query_has_subquery_or_set_op(sql_query: str) -> Tuple[bool, List[str]]:
    """
    assume sql_query argument is lowercase.
...
def get_difficulty_type(sql: str) -> str:
    sql = sql.lower()

    sql_has_join = deprecated_query_has_join(sql)
    sql_has_subquery, subqueries = deprecated_query_has_subquery_or_set_op(sql)

    if sql_has_join:
        return 'non-nested'
    elif sql_has_subquery:
        return 'nested'
    else:
        return 'easy'

def get_sql_difficulty_types_list(sql: str) -> List[str]:
    sql = sql.lower()
    types_list = []
    
    if query_has_join(sql):
        types_list.append('join')
    
    if query_has_subquery_or_set_op(sql)[0]:
        types_list.append('nested')
...
if __name__ == '__main__':
    ...
    print(deprecated_query_has_subquery_or_set_op(sql_query1))

    sql_query2 = "SELECT * FROM table1 INNER JOIN table2 ON table1.id = table2.id"
    sql_query2 = sql_query2.lower()
    print(deprecated_query_has_join(sql_query2))
    ...
    bool_v, subqueries = deprecated_query_has_subquery_or_set_op(sql_statement.lower())
    print(bool_v)
    print(subqueries)
