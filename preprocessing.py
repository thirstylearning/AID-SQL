...
from utils.schemas import PreprocessedDatasetSample
from utils.sql_parse_classify import get_sql_difficulty_types_list
from utils.utils import eval_spider_difficulty
...
def get_db_schemas(all_db_infos):
    ...
    for db in all_db_infos:
        ...
        # record primary keys
        pk_column_idx: Union[int, List[int]] # bird table have union primary keys, so we need to handle it
        for pk_column_idx in db["primary_keys"]:
            ...
            pk_column_name_original: Union[str, List[str]]
            if isinstance(pk_column_idx, int):
                pk_column_name_original = column_names_original[pk_column_idx][1]
                pk_column_name_original = pk_column_name_original.lower()
            else: # List[int]
                pk_column_name_original = [column_names_original[_idx][1] for _idx in pk_column_idx]
                pk_column_name_original = [_col.lower() for _col in pk_column_name_original]

            primary_keys.append(
                {
                    "table_name_original": pk_table_name_original.lower(),
                    "column_name_original": pk_column_name_original
                }
            )
...
def get_sample_schema_links(table_labels: List, column_labels: List, db_schema: List) -> List:
    """get a sample's schema links"""
    schema_links = []
    for table_idx, table_label in enumerate(table_labels):
        if table_label == 0:
            continue
        table_name: str = db_schema[table_idx]['table_name_original']
        for column_idx, column_label in enumerate(column_labels[table_idx]):
            if column_label == 0:
                continue
            column_name: str = db_schema[table_idx]['column_names_original'][column_idx]
            schema_link: str = f'{table_name}.{column_name}'
            schema_links.append(schema_link)
        schema_links.append(table_name)

    return schema_links

    
def get_dev_id_prefix(dataset_name: str) -> str:
    if dataset_name == "spider":
        return "spider_dev"
    elif dataset_name == "bird":
        return "bird_dev"
    elif dataset_name == "spider_test":
        return "spider_test"
    else:
        raise ValueError(f"Unknown dataset name: {dataset_name}")

def get_dataset_sql_key(dataset_name: str) -> str:
    if dataset_name == "spider" or dataset_name == "spider_test":
        return "query"
    elif dataset_name == "bird":
        return "SQL"
    else:
...
def main(opt):
    ...
    dev_dataset_flag = True if "dev" in opt.input_dataset_path or "test" in opt.dataset_name else False
    dataset_sql_key: str = get_dataset_sql_key(opt.dataset_name)

    db_schemas = get_db_schemas(all_db_infos)

    preprocessed_dataset = []

    for _idx, (natsql_data, data) in tqdm(enumerate(zip(natsql_dataset, dataset))):
        ...
        if opt.mode == "test":
            ...
            sql = data[dataset_sql_key].strip()
        ...
        # not use spider difficulty
        preprocessed_data["spider_difficulty"] = None
        # preprocessed_data["spider_difficulty"] = eval_spider_difficulty(data["sql"])

        # parse sql query to determine the difficulty type
        sql_difficulty_types: List[str] = get_sql_difficulty_types_list(preprocessed_data['norm_sql'])

        # save sub-queries---not used in the current implementation
        preprocessed_data['sub_queries'] = []

        preprocessed_data['difficulty_type'] = sql_difficulty_types

        preprocessed_data['schema_links'] = get_sample_schema_links(table_labels=preprocessed_data['table_labels'],
                                                                    column_labels=preprocessed_data['column_labels'],
                                                                    db_schema=preprocessed_data['db_schema'])

        preprocessed_data_sample: PreprocessedDatasetSample = PreprocessedDatasetSample(**preprocessed_data)

        dev_id_prefix = get_dev_id_prefix(opt.dataset_name)
        
        if dev_dataset_flag:
            _prepro_id: int = data["question_id"] if "question_id" in data.keys() else _idx
            preprocessed_data_sample.id = f"{dev_id_prefix}_{_prepro_id}"
        preprocessed_dataset.append(preprocessed_data_sample.dict())
