...
from modeling.strided_tensor import StridedTensor
...
class ColBERT(BaseColBERT):
    ...
    def forward(self, Q, D):
        ...
        if (
            self.colbert_config.use_ib_negatives
            or self.colbert_config.use_self_negatives
            or self.colbert_config.use_self_easy_negatives
        ):
            ib_self_loss = self.compute_ib_self_loss(Q, D, D_mask)
            return scores, ib_self_loss

        return scores

    def compute_ib_self_loss(self, Q, D, D_mask):
        ...
        ib_loss, self_loss, self_easy_loss = None, None, None

        if self.colbert_config.use_ib_negatives:
            assert False, "do not use ib_negatives"
            ...
            ib_scores = scores[flatten(all_except_self_negatives)]
            ib_scores = ib_scores.view(Q.size(0), -1)  # D.size(0) - self.colbert_config.nway + 1)

            ib_labels = torch.arange(0, Q.size(0), device=ib_scores.device) * (self.colbert_config.nway)
            ib_loss = torch.nn.CrossEntropyLoss()(ib_scores, ib_labels)

        if self.colbert_config.use_self_negatives:
            self_negatives = [
                list(range(qidx * D.size(0) + nway * qidx, qidx * D.size(0) + nway * (qidx + 1)))
                for qidx in range(Q.size(0))
            ]
            self_scores = scores[flatten(self_negatives)]
            self_scores = self_scores.view(Q.size(0), -1)

            self_labels = torch.zeros(Q.size(0), device=self_scores.device, dtype=torch.long)
            self_loss = torch.nn.CrossEntropyLoss()(self_scores, self_labels)

        if self.colbert_config.use_self_easy_negatives:
            easy_start_offset = self.colbert_config.nway - 1 - self.colbert_config.easy_nway
            self_easy_negatives = [
                [qidx*D.size(0) + qidx*nway] + 
                list(range(qidx*D.size(0) + qidx*nway + 1 + easy_start_offset, qidx*D.size(0) + qidx*nway + nway))
                for qidx in range(Q.size(0))
            ]

            self_easy_scores = scores[flatten(self_easy_negatives)]
            self_easy_scores = self_easy_scores.view(Q.size(0), -1)

            self_easy_labels = torch.zeros(Q.size(0), device=self_easy_scores.device, dtype=torch.long)
            self_easy_loss = torch.nn.CrossEntropyLoss()(self_easy_scores, self_easy_labels)

        return ib_loss, self_loss, self_easy_loss
